...
export default function EventDetailModal({ event, onClose, onDelete }: Props) {
  return (
    <div onClick={onClose} style={{ position:"fixed",inset:0,zIndex:50,background:"rgba(10,10,10,0.18)",backdropFilter:"blur(2px)",display:"flex",alignItems:"center",justifyContent:"center" }}>
      <div onClick={(e) => e.stopPropagation()} style={{ background:"#fff",borderRadius:20,padding:26,width:360,boxShadow:"0 32px 80px rgba(17,22,30,0.12)",border:"1px solid #ece8e1" }}>
        <div style={{ display:"flex",alignItems:"flex-start",gap:14,marginBottom:20 }}>
          <div style={{ width:4,minHeight:48,borderRadius:2,background:event.color,flexShrink:0,marginTop:2 }} />
          <div style={{ flex:1 }}>
        ...
        <button onClick={() => onDelete(event.id)} style={{ width:"100%",padding:"11px 0",background:"#fae6e5",border:"1px solid #f2c3c1",borderRadius:12,color:"#b86060",fontSize:13,fontWeight:700,cursor:"pointer" }}>
          Delete Event
        </button>
      </div>
    </div>
